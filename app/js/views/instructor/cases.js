      ...
      el('div', { class: 'row', style: 'gap: 15px; align-items: end; flex-wrap: wrap;' }, [
        ...
        el('div', { class: 'col' }, [
          ...
          (() => {
            ...
            ['knee', 'shoulder', 'low back', 'neck', 'ankle', 'hip', 'elbow', 'wrist'].forEach(
              (r) => {
  ...
  function renderCases() {
    ...
    let filteredCases = allCases.filter((c) => {
      ...
      if (
        ...
        !title.toLowerCase().includes(currentFilters.search.toLowerCase())
      )
        return false;
      if (currentFilters.setting !== 'all' && caseData.setting !== currentFilters.setting)
        return false;
      if (currentFilters.diagnosis !== 'all' && caseData.diagnosis !== currentFilters.diagnosis)
        return false;
      if (currentFilters.acuity !== 'all' && caseData.acuity !== currentFilters.acuity)
        return false;
      if (
        currentFilters.region !== 'all' &&
        !(caseData.regions || []).includes(currentFilters.region)
      )
        return false;
      return true;
    });
