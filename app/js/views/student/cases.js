    ...
    if (!Array.isArray(cases)) {
      app.append(
        el(
          'div',
          { class: 'panel error' },
          'Could not load cases. Please check the console for details.',
        ),
      );
      return;
    }
          ...
          sections.forEach((section) => {
            ...
            if (typeof sectionData === 'string') {
              ...
              if (section === 'subjective') {
                ...
                const hasSubjectiveContent = Object.values(sectionData).some(
                ...
                const hasAssessmentContent = Object.values(sectionData).some(
                  (value) => value && typeof value === 'string' && value.trim().length > 0,
                );
      ...
      const table = el('table', { class: 'table cases-table' }, [
        ...
        el(
          ...
          filtered.map((c) => {
            ...
            const buttonText =
              evalDraft && evalDraft.hasContent ? 'Continue Working' : 'Start Case';
                      ...
                      <div class="section">
                        ...
                        <p>${(() => {
                          ...
                          if (draft.subjective.chiefComplaint) {
                            subjectiveContent.push(
                              `<strong>Chief Concern:</strong> ${draft.subjective.chiefComplaint}`,
                            );
                          }
                          if (draft.subjective.historyOfPresentIllness) {
                            subjectiveContent.push(
                              `<strong>History of Present Illness:</strong> ${draft.subjective.historyOfPresentIllness}`,
                            );
                          }
                          if (draft.subjective.painLocation) {
                            subjectiveContent.push(
                              `<strong>Pain Location:</strong> ${draft.subjective.painLocation}`,
                            );
                          }
                          if (draft.subjective.painScale) {
                            subjectiveContent.push(
                              `<strong>Pain Scale:</strong> ${draft.subjective.painScale}/10`,
                            );
                          }
                          if (draft.subjective.painQuality) {
                            subjectiveContent.push(
                              `<strong>Pain Quality:</strong> ${draft.subjective.painQuality}`,
                            );
                          }
                          if (draft.subjective.aggravatingFactors) {
                            subjectiveContent.push(
                              `<strong>Aggravating Factors:</strong> ${draft.subjective.aggravatingFactors}`,
                            );
                          }
                          if (draft.subjective.easingFactors) {
                            subjectiveContent.push(
                              `<strong>Easing Factors:</strong> ${draft.subjective.easingFactors}`,
                            );
                          }
                          if (draft.subjective.functionalLimitations) {
                            subjectiveContent.push(
                              `<strong>Functional Limitations:</strong> ${draft.subjective.functionalLimitations}`,
                            );
                          }
                          if (draft.subjective.priorLevel) {
                            subjectiveContent.push(
                              `<strong>Prior Level of Function:</strong> ${draft.subjective.priorLevel}`,
                            );
                          }
                          if (draft.subjective.patientGoals) {
                            subjectiveContent.push(
                              `<strong>Patient Goals:</strong> ${draft.subjective.patientGoals}`,
                            );
                          }
                          if (draft.subjective.medicationsCurrent) {
                            subjectiveContent.push(
                              `<strong>Current Medications:</strong> ${draft.subjective.medicationsCurrent}`,
                            );
                          }
                          if (draft.subjective.redFlags) {
                            subjectiveContent.push(
                              `<strong>Red Flags/Screening:</strong> ${draft.subjective.redFlags}`,
                            );
                          }
                          if (draft.subjective.additionalHistory) {
                            subjectiveContent.push(
                              `<strong>Additional History:</strong> ${draft.subjective.additionalHistory}`,
                            );
                          }

                          return subjectiveContent.length > 0
                            ? subjectiveContent.join('<br><br>')
                            : 'Patient subjective findings not documented at this time.';
                        })()}</p>
                      </div>
                        ...
                        const sections = [
                          {
                            ...
                            placeholder:
                              'Clinical assessment and diagnostic reasoning not documented at this time.',
                          },
                          {
                            name: 'GOALS',
                            content: draft.goals,
                          ...
                          if (
                            ...
                            if (section.content.primaryImpairments) {
                              assessmentContent.push(
                                `<strong>Primary Impairments:</strong> ${section.content.primaryImpairments}`,
                              );
                            }
                            if (section.content.bodyFunctions) {
                              assessmentContent.push(
                                `<strong>Body Functions & Structures:</strong> ${section.content.bodyFunctions}`,
                              );
                            }
                            if (section.content.activityLimitations) {
                              assessmentContent.push(
                                `<strong>Activity Limitations:</strong> ${section.content.activityLimitations}`,
                              );
                            }
                            if (section.content.participationRestrictions) {
                              assessmentContent.push(
                                `<strong>Participation Restrictions:</strong> ${section.content.participationRestrictions}`,
                              );
                            }
                            if (section.content.ptDiagnosis) {
                              assessmentContent.push(
                                `<strong>PT Diagnosis:</strong> ${section.content.ptDiagnosis}`,
                              );
                            }
                            if (section.content.clinicalReasoning) {
                              assessmentContent.push(
                                `<strong>Clinical Reasoning:</strong> ${section.content.clinicalReasoning}`,
                              );
                            }
                            contentText =
                              assessmentContent.length > 0
                                ? assessmentContent.join('<br><br>')
                                : null;
                          }
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith('draft_blank')) {
          try {
            ...
            const title =
              (data && data.noteTitle && data.noteTitle.trim()) ||
              key.replace('draft_', '').replace('_eval', '');
