...
export function createRegionalAssessment(regionKey, assessmentData, onChange) {
  ...
  // ROM Section
  const romSection = createRomSection(
    regionKey,
    region,
    assessmentData.rom,
    (romData) => {
  ...
  // MMT Section
  const mmtSection = createMmtSection(
    regionKey,
    region,
    assessmentData.mmt,
    (mmtData) => {
...
export function createMultiRegionalAssessment(allAssessmentData, onChange) {
    ...
    try {
      ...
      promKeys.forEach(k => {
        ...
        if (row && typeof row === 'object') {
          const isCapsular = typeof row.endfeel === 'string' && row.endfeel.toLowerCase() === 'capsular';
          if (isCapsular) { row.endfeel = ''; changed = true; }
        }
      });
      if (changed) {
  ...
  const regionLabel = el('p', { style: 'margin-bottom: 10px; font-weight: 500; font-size: 14px;' }, 'Select regions to assess:');
  ...
  const specialTestsContainer = el('div', { class: 'special-tests-container', style: 'margin-bottom: 30px;' });
  ...
  promTable = createEditableTable({
        ...
        onChange: (newData) => {
          ...
          const currentRowIds = Object.keys(groups).map(n => n.toLowerCase().replace(/\s+/g, '-'));
          ...
          const prunedExisting = Array.from(existingExcluded).filter(id => currentRowIds.includes(id));
