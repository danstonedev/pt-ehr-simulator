...
export const TreatmentPlan = {
  ...
  section.append(el('h4', { class: 'subsection-title' }, 'Plan of Care'));
    ...
    section.append(textAreaField({
      label: 'Primary Treatment Approach',
      value: data.treatmentPlan,
      onChange: v => updateField('treatmentPlan', v),
      placeholder: 'Describe overall treatment philosophy and approach (e.g., movement-based intervention, manual therapy focus, functional training...)'
    }));
  ...
  // Treatment frequency and duration (moved below Patient Education)
    ...
    scheduleRow.append(selectField({
      label: 'Frequency',
  ...
  // We'll render the schedule inside its own In-Clinic Treatment Plan subsection below
    ...
    const exerciseTable = createEditableTable({
      ...
      columns: [
        { field: 'exerciseText', label: 'Exercise/Activity', width: '100%', type: 'textarea', rows: 1, placeholder: 'Describe the intervention, dosage, or response...' }
      ],
