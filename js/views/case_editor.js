...
async function renderCaseEditor(app, qs, isFacultyMode) {
  ...
  function getHeaderOffsetPx() {
    ...
    const stickyH = parseInt((cs.getPropertyValue('--sticky-header-h') || '').replace('px','').trim(), 10);
  ...
  if (!caseId) {
    app.innerHTML = '';
    app.append(el('div', {class:'panel error'}, [
      el('h2', {}, 'Missing Case ID'),
      el('p', {}, `No case ID provided in URL. Expected format: #/student/editor?case=CASE_ID&v=0&encounter=eval`)
    ]));
    return;
  }
  ...
  let needsInitialAnchorScroll = !needsInitialPercentScroll && !!initialAnchorParam && isValidSection(active);
  ...
  navigate(`${base}?case=${caseId}&v=${version}&encounter=${encounter}${from}${sectionQS}${anchorQS}${spQS}`);
    },
    onExitPreview: () => {
      ...
      const sectionQS = `&section=${encodeURIComponent(active)}`;
      const anchorQS = anchorId ? `&anchor=${encodeURIComponent(anchorId)}` : '';
  const sp = Number.isFinite(initialScrollPercent) ? initialScrollPercent : getSectionScrollPercent();
  const spQS = `&sp=${sp.toFixed(4)}`;
  navigate(`#/instructor/editor?case=${caseId}&v=${version}&encounter=${encounter}${sectionQS}${anchorQS}${spQS}`);
    }
  });
  ...
  function performInitialScrollIfNeeded(currentSectionId) {
    ...
    if (needsInitialPercentScroll && Number.isFinite(initialScrollPercent)) {
      ...
      afterNextLayout(() => { if (!okP) okP = scrollToPercentExact(initialScrollPercent); });
      setTimeout(() => {
        ...
        if (!okP) {
          ...
          if (initialAnchorParam) {
            ...
            afterNextLayout(() => { if (!ok) ok = scrollToAnchorExact(initialAnchorParam); });
            setTimeout(() => {
  ...
  caseData: { ...c, ...draft, editorSettings: (c.editorSettings || draft.editorSettings) },
     ...
     if(s==='subjective'){ 
       const subjectiveSection = createSubjectiveSection(
         draft.subjective, 
         (data) => {
 ...
 performInitialScrollIfNeeded(s);
       ...
       window.refreshInterventionCard = (rowId) => {
         ...
         if (active === 'plan') {
           ...
           if (planContent) {
             const refreshedPlanSection = createPlanSection(
               draft.plan,
               (data) => {
                 draft.plan = data;
                 save();
